...
public partial class MyWorkspace_MyInvitedGroups : workareabase
{
    ...
    protected void Process_Decline()
    {
        ...
        if ((aUid != null) && aUid.Length > 0)
        {
            for (int i = 0; i <= (aUid.Length - 1); i++)
            {
                ...
                {
                    m_refCommunityGroupApi.DeclineGroupInvite(this.m_refContentApi.UserId, Convert.ToInt64(aUid[i]));
                }
            }
        }
        Response.Redirect("MyInvitedGroups.aspx", false);
    }
    protected void Process_Accept()
    {
        string[] aUid = new List<string>().ToArray();
        aUid = Strings.Split(Request.Form["req_deleted_users"], ",", -1, 0);
        if ((aUid != null) && aUid.Length > 0)
        {
            for (int i = 0; i <= (aUid.Length - 1); i++)
            {
                if (Information.IsNumeric(aUid[i].Trim()))
                {
                    m_refCommunityGroupApi.AcceptGroupInvite(this.m_refContentApi.UserId, Convert.ToInt64(aUid[i]));
                }
            }
        }
        Response.Redirect("MyInvitedGroups.aspx", false);
    ...
    public void SetLabels()
    {
        ...
        {
            default: // "viewall"
                this.AddButtonwithMessages(this.m_refContentApi.AppImgPath + "groupinviteapprove.gif", "#", "alt accept my cgroup invites", "btn accept my cgroup invites", " onclick=\"javascript:return CheckAccept();\" ", StyleHelper.InviteButtonCssClass, true);
                this.AddButtonwithMessages(this.m_refContentApi.AppImgPath + "btn_delete-nm.gif", "#", "alt decline my cgroup invites", "btn decline my cgroup invites", " onclick=\"javascript:return CheckDelete();\" ", StyleHelper.DeclineButtonCssClass);
                this.AddHelpButton("viewmycommunitygroupinvites");
                this.SetTitleBarToMessage("lbl myworkspace invited groups");
                break;
        }
    }
    ...
    protected void BuildJS()
    {
        ...
        sbJS.Append(" 			} ").Append(Environment.NewLine);
        sbJS.Append(" 	    } ").Append(Environment.NewLine);
        sbJS.Append(" 	    if (bCheck) { if (confirm(\'").Append(GetMessage("js confirm decline cgroup invites")).Append("\')) { document.getElementById(\'isDeleted\').value = \'1\'; document.form1.submit(); } else { bCheck = false; } } ").Append(Environment.NewLine);
        sbJS.Append(" 	    else { alert(\'").Append(GetMessage("js err please select cgroup invites")).Append("\'); } ").Append(Environment.NewLine);
        sbJS.Append(" 	    return bCheck; ").Append(Environment.NewLine);
        sbJS.Append("     } ").Append(Environment.NewLine);

        sbJS.Append("     function CheckAccept() ").Append(Environment.NewLine);
        sbJS.Append("     { ").Append(Environment.NewLine);
        sbJS.Append("         var bCheck = false; ").Append(Environment.NewLine);
        sbJS.Append("         for (var i=0;i<document.forms[0].elements.length;i++){ ").Append(Environment.NewLine);
        sbJS.Append(" 	        var e = document.forms[0].elements[i]; ").Append(Environment.NewLine);
        ...
        sbJS.Append(" 			} ").Append(Environment.NewLine);
        sbJS.Append(" 	    } ").Append(Environment.NewLine);
        sbJS.Append(" 	    if (bCheck) { if (confirm(\'").Append(GetMessage("js confirm accept cgroup invites")).Append("\')) { document.getElementById(\'isDeleted\').value = \'2\'; document.form1.submit(); } else { bCheck = false; } } ").Append(Environment.NewLine);
        sbJS.Append(" 	    else { alert(\'").Append(GetMessage("js err please select cgroup invites")).Append("\'); } ").Append(Environment.NewLine);
        sbJS.Append(" 	    return bCheck; ").Append(Environment.NewLine);
        sbJS.Append("     } ").Append(Environment.NewLine);
