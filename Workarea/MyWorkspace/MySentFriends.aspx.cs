...
public partial class MyWorkspace_MySentFriends : workareabase
{
    protected FriendInvitation[] m_aInvitations = new List<FriendInvitation>().ToArray();
    ...
    protected void Process_Remove()
    {
        ...
        if ((aUid != null) && aUid.Length > 0)
        {
            for (int i = 0; i <= (aUid.Length - 1); i++)
            {
                ...
                {
                    m_refFriendsApi.DeleteSentFriendRequest(Convert.ToInt64(aUid[i]));
                }
                else
                {
                    if (aUid[i].Trim().Length > 0)
                    {
                        m_refFriendsApi.DeleteSentFriendRequestByEmail(aUid[i].Trim());
                    }
                }
            }
        }
        Response.Redirect("MySentFriends.aspx", false);
    }
    ...
    {
        this.AddButtonwithMessages(this.m_refContentApi.AppImgPath + "btn_delete-nm.gif", "#", "alt remove users from my friends", "btn remove users from my friends", " onclick=\"javascript:return CheckDelete();\" ", StyleHelper.RemoveButtonCssClass, true);
        this.AddButtonwithMessages(this.m_refContentApi.AppImgPath + "mail_forward.gif", "#", "lbl alt resend invite new friends", "lbl alt resend invite new friends", " onclick=\"javascript:return ShowResendUI();\" ", StyleHelper.EmailButtonCssClass);
        this.AddHelpButton("viewmysentfriends");
        this.SetTitleBarToMessage("lbl my sent invites");
        lit_optmsglbl.Text = GetMessage("lbl optional message");
        btn_resendInvites.Text = GetMessage("lbl resend invite new friends");
        btn_resendInvites.Attributes.Add("onclick", "return ResendBtnClicked();");
    ...
    protected void BuildJS()
    {
        ...
        sbJS.Append("       var itemsChecked = VerifyItemsChecked(); ").Append(Environment.NewLine);
        sbJS.Append(" 	    if (itemsChecked) { if (confirm(\'").Append(GetMessage("js confirm remove friend from my friends")).Append("\')) { document.getElementById(\'isDeleted\').value = \'1\'; document.forms[0].submit(); } else { itemsChecked = false; } } ").Append(Environment.NewLine);
        sbJS.Append(" 	    else { alert(\'").Append(GetMessage("js err my friends please select user remove")).Append("\'); } ").Append(Environment.NewLine);
        sbJS.Append(" 	    return (itemsChecked); ").Append(Environment.NewLine);
        sbJS.Append("     } ").Append(Environment.NewLine);

        sbJS.Append("     function ShowResendUI() ").Append(Environment.NewLine);
        sbJS.Append("     { ").Append(Environment.NewLine);
        sbJS.Append("       var resendUiObj = document.getElementById(\'block_resendUI\'); " + Environment.NewLine);
        sbJS.Append(" 	    if (resendUiObj) {").Append(Environment.NewLine);
        sbJS.Append(" 	        resendUiObj.style.display = \'block\';").Append(Environment.NewLine);
        sbJS.Append(" 	    }").Append(Environment.NewLine);
        sbJS.Append("     } ").Append(Environment.NewLine);


        sbJS.Append("     function ResendBtnClicked() ").Append(Environment.NewLine);
        sbJS.Append("     { ").Append(Environment.NewLine);
        sbJS.Append("       var itemsChecked = VerifyItemsChecked(); ").Append(Environment.NewLine);
        sbJS.Append(" 	    if (!itemsChecked) {").Append(Environment.NewLine);
        sbJS.Append(" 	        alert(\'").Append(GetMessage("js err please select user")).Append("\');").Append(Environment.NewLine);
        sbJS.Append(" 	    }").Append(Environment.NewLine);
        sbJS.Append(" 	    return (itemsChecked); ").Append(Environment.NewLine);
        sbJS.Append("     } ").Append(Environment.NewLine);

        sbJS.Append("     function VerifyItemsChecked() ").Append(Environment.NewLine);
        sbJS.Append("     { ").Append(Environment.NewLine);
    ...
    protected void Click_ResendInvites(object sender, System.EventArgs e)
    {
        InvitationSendRequestData inviteData = null;
        Ektron.Cms.Community.FriendsAPI friendsApi = null;

        try
        {
            string[] aUid = new List<string>().ToArray();
            inviteData = new InvitationSendRequestData();
            ...
            if ((aUid != null) && aUid.Length > 0)
            {
                for (int i = 0; i <= (aUid.Length - 1); i++)
                {
                    if (Information.IsNumeric(aUid[i].Trim()))
                    {
                        inviteData.UserIds.Add(Convert.ToInt64(aUid[i])); //refFriendsApi.AddPendingFriend(aUid(i))
                    }
                    else
                    {
                        if (aUid[i].Trim().Length > 0)
                        {
                            inviteData.EmailAddresses.Add(aUid[i].Trim());
                        }
                    }
                }
            }

            if (inviteData.EmailAddresses.Count > 0 || inviteData.UserIds.Count > 0)
            {
                inviteData.OptionalText = (string)tb_optmsg.Text;

                friendsApi.InviteUsers(inviteData);
            }
