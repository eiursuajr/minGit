...
public partial class MyWorkspace_MyPendingGroups : workareabase
{
    ...
    {
        int m_intTotalGroups = 0;
        m_aGroups = m_refCommunityGroupApi.GetPendingCommunityGroupsForUser(this.m_refContentApi.UserId, m_intCurrentPage, System.Convert.ToInt32(m_PageSize > 0 ? m_PageSize : 0), ref m_intTotalPages, ref m_intTotalGroups);
    ...
    protected void Process_Remove()
    {
        ...
        if ((aUid != null) && aUid.Length > 0)
        {
            for (int i = 0; i <= (aUid.Length - 1); i++)
            {
                ...
                {
                    m_refCommunityGroupApi.CancelJoinRequestForCommunityGroup(Convert.ToInt64(aUid[i]), this.m_refContentApi.UserId);
                }
            }
        }
        Response.Redirect("MyPendingGroups.aspx", false);
    }
    ...
    public void SetLabels()
    {
        ...
        {
            default: // "viewall"
                this.AddButtonwithMessages(this.m_refContentApi.AppImgPath + "btn_delete-nm.gif", "#", "alt remove my pending cgroup", "btn remove my pending cgroup", " onclick=\"javascript:return CheckDelete();\" ", StyleHelper.RemoveButtonCssClass, true);
                this.AddHelpButton("viewmypendingcommunitygroups");
                this.SetTitleBarToMessage("lbl myworkspace pending groups");
                break;
        }
    }
    ...
    protected void BuildJS()
    {
        ...
        sbJS.Append(" 			} ").Append(Environment.NewLine);
        sbJS.Append(" 	    } ").Append(Environment.NewLine);
        sbJS.Append(" 	    if (bCheck) { if (confirm(\'").Append(GetMessage("js confirm leave pending cgroup")).Append("\')) { document.getElementById(\'isDeleted\').value = \'1\'; document.form1.submit(); } else { bCheck = false; } } ").Append(Environment.NewLine);
        sbJS.Append(" 	    else { alert(\'").Append(GetMessage("js err please select pending cgroup leave")).Append("\'); } ").Append(Environment.NewLine);
