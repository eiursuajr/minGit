 ...
 * 
 */
$ektron.ptTimeSelect._ptTimeSelectInit = function () {
	Ektron.ready(
		function () {
			//if the html is not yet created in the document, then do it now
			if (!$ektron('#ptTimeSelectCntr').length) {
				$ektron("body").append(
						'<div id="ptTimeSelectCntr" style="display:none;">'
					+	'		<div class="ui-widget ui-widget-content ui-corner-all">'
					+	'		<div class="ui-widget-header ui-corner-all">'
					+	'			<div id="ptTimeSelectCloseCntr" style="float: right;">'
					+	'				<a href="javascript: void(0);" onclick="$ektron.ptTimeSelect.closeCntr();" '
					+	'						onmouseover="$ektron(this).removeClass(\'ui-state-default\').addClass(\'ui-state-hover\');" '
					+	'						onmouseout="$ektron(this).removeClass(\'ui-state-hover\').addClass(\'ui-state-default\');"'
					...
					+	'			<a href="javascript: void(0);" onclick="$ektron.ptTimeSelect.setTime()"'
					+	'					onmouseover="$ektron(this).removeClass(\'ui-state-default\').addClass(\'ui-state-hover\');" '
					+	'						onmouseout="$ektron(this).removeClass(\'ui-state-hover\').addClass(\'ui-state-default\');"'
					+	'						class="ui-corner-all ui-state-default">'
					+	'				SET'
				...
				var e = $ektron('#ptTimeSelectCntr');

				// Add the events to the functions
				e.find('.ptTimeSelectMin')
					.bind("click", function(){
						$ektron.ptTimeSelect.setMin($ektron(this).text());
	 				});
				
				e.find('.ptTimeSelectHr')
					.bind("click", function(){
						$ektron.ptTimeSelect.setHr($ektron(this).text());
	 				});
				
				$ektron(document).mousedown($ektron.ptTimeSelect._doCheckMouseClick);			
			}//end if
		}
	);
 ...
 * 		none
 */
$ektron.ptTimeSelect.setHr = function(h) {
	if (	h.toLowerCase() == "am"
		||	h.toLowerCase() == "pm"
	) {
		$ektron('#ptTimeSelectUserSelAmPm').empty().append(h);
	} else {
		$ektron('#ptTimeSelectUserSelHr').empty().append(h);
	}
}/* END setHr() function */
 ...
 * 		none
 */
$ektron.ptTimeSelect.setMin = function(m) {
	$ektron('#ptTimeSelectUserSelMin').empty().append(m);
}/* END setMin() function */
 ...
 * 		none
 */
$ektron.ptTimeSelect.setTime = function() {
	var tSel = $ektron('#ptTimeSelectUserSelHr').text()
				+ ":"
				+ $ektron('#ptTimeSelectUserSelMin').text()
				+ " "
				+ $ektron('#ptTimeSelectUserSelAmPm').text()
	$ektron(".isPtTimeSelectActive").val(tSel);
	this.closeCntr();
 ...
 * 
 */
$ektron.ptTimeSelect.openCntr = function(ele) {
    $ektron.ptTimeSelect.closeCntr()
    $ektron(".isPtTimeSelectActive").removeClass("isPtTimeSelectActive");
    var cntr = $ektron("#ptTimeSelectCntr");
    var i = $ektron(ele).eq(0).addClass("isPtTimeSelectActive");
    ...
    //get height, hide, reposition so entire cntr is visible, then slidedown

    var tpWidth = cntr.outerWidth();
    var tpHeight = cntr.outerHeight();
	var inputWidth = i.outerWidth();
	var inputHeight = i.outerHeight();
	var viewWidth = (window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) + $(document).scrollLeft();
	var viewHeight = (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) + $(document).scrollTop();

	style.top = (i.offset().top + inputHeight);

	// now check if datepicker is showing outside window viewport - move to a better place if so.
	style.left -= (style.left + tpWidth > viewWidth && viewWidth > tpWidth) ? Math.abs(style.left + tpWidth - viewWidth) : 0;
	style.top -= (style.top + tpHeight > viewHeight && viewHeight > tpHeight) ? Math.abs(style.top + tpHeight + inputHeight * 2 - viewHeight) : 0;
 ...
 * 
 */
$ektron.ptTimeSelect.closeCntr = function(i) {
	if ($ektron("#ptTimeSelectCntr:visible").length) {
		$ektron('#ptTimeSelectCntr')
			.slideUp("fast", function(){
				$ektron('#ptTimeSelectCntr').removeClass().css("width", "");
				if (!i) {
					i = $ektron(".isPtTimeSelectActive");
				}
				if (i) {
...
$ektron.ptTimeSelect._doCheckMouseClick = function(ev){
	if (!$ektron("#ptTimeSelectCntr:visible").length) {
		return;
	}
	if (!$ektron(ev.target).closest("#ptTimeSelectCntr").length){
		$ektron.ptTimeSelect.closeCntr();
	}
 ...
 * 
 */
$ektron.fn.ptTimeSelect = function (opt) {
	this.each(function(){
		...
		var thisOpt = {};
		thisOpt = $.extend(thisOpt, $ektron.ptTimeSelect.options, opt);
		...
		if (thisOpt.popupImage || !thisOpt.onFocusDisplay) {
			var img = $ektron(
					...
					+	'$ektron.ptTimeSelect.openCntr($ektron(this).data(\'ptTimeSelectEle\'));">'
		...
		if (thisOpt.onFocusDisplay){
			e.focus(function(){
				$ektron.ptTimeSelect.openCntr(this);
			});
		}
		return this;
	});
