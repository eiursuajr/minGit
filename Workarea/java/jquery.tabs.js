...
/**
 * Tabs - ektjq plugin for accessible, unobtrusive tabs
 ...
 *                              default value becomes true. @see ektj$.ajaxHistory.initialize
 ...
 */
ektj$.fn.tabs = function(initial, settings) {

    // settings
    if (typeof initial == 'object') settings = initial; // no initial tab given but a settings object
    settings = ektj$.extend({
        initial: (initial && typeof initial == 'number' && initial > 0) ? --initial : 0,
        disabled: null,
        bookmarkable: ektj$.ajaxHistory ? true : false,
        remote: false,
        spinner: 'Loading&#8230;',
    ...
    ektj$.browser.msie6 = ektj$.browser.msie && (ektj$.browser.version && ektj$.browser.version < 7 || /MSIE 6.0/.test(navigator.userAgent)); // do not check for 6.0 alone, userAgent in Windows Vista has "Windows NT 6.0"
    ...
    return this.each(function() {
        ...
        // setup nav
        var nav = ektj$('ul.' + settings.navClass, container);
        nav = nav.size() && nav || ektj$('>ul:eq(0)', container); // fallback to default structure
        var tabs = ektj$('a', nav);

        // prepare remote tabs
        if (settings.remote) {
            tabs.each(function() {
                var id = settings.hashPrefix + (++ektj$.tabs.remoteCount), hash = '#' + id, url = this.href;
                this.href = hash;
                ektj$('<div id="' + id + '" class="' + settings.containerClass + '"></div>').appendTo(container);

                ektj$(this).bind('loadRemoteTab', function(e, callback) {
                    var ektj$ektj$ = ektj$(this).addClass(settings.loadingClass), span = ektj$('span', this)[0], tabTitle = span.innerHTML;
                    if (settings.spinner) {
                    ...
                    setTimeout(function() { // Timeout is again required in IE, "wait" for id being restored
                        ektj$(hash).load(url, function() {
                            if (settings.spinner) {
                                span.innerHTML = tabTitle; // WARNING: html(...) crashes Safari with jQuery 1.1.2
                            }
                            ektj$ektj$.removeClass(settings.loadingClass);
                            callback && callback();
                        });
                    }, 0);
                });
        ...
        // set up containers
        var containers = ektj$('div.' + settings.containerClass, container);
        containers = containers.size() && containers || ektj$('>' + settings.tabStruct, container); // fallback to default structure

        // attach classes for styling if not present
        nav.is('.' + settings.navClass) || nav.addClass(settings.navClass);
        containers.each(function() {
            var ektj$ektj$ = ektj$(this);
            ektj$ektj$.is('.' + settings.containerClass) || ektj$ektj$.addClass(settings.containerClass);
        });

        // try to retrieve active tab from class in HTML
        var hasSelectedClass = ektj$('li', nav).index( ektj$('li.' + settings.selectedClass, nav)[0] );
        ...
        if (location.hash) {
            tabs.each(function(i) {
                if (this.hash == location.hash) {
                    ...
                    if ((ektj$.browser.msie || ektj$.browser.opera) && !settings.remote) {
                        var toShow = ektj$(location.hash);
        ...
        if (ektj$.browser.msie) {
            unFocus(); // fix IE focussing bottom of the page for some unknown reason
        }

        // highlight tab accordingly
        containers.filter(':eq(' + settings.initial + ')').show().end().not(':eq(' + settings.initial + ')').addClass(settings.hideClass);
        ektj$('li', nav).removeClass(settings.selectedClass).eq(settings.initial).addClass(settings.selectedClass); // we need to remove classes eventually if hash takes precedence over class
        ...
        if (settings.fxAutoHeight) {
            ...
            var _setAutoHeight = function(reset) {
                ...
                var heights = ektj$.map(containers.get(), function(el) {
                    var h, jq = ektj$(el);
                    if (reset) {
                        if (ektj$.browser.msie6) {
            ...
            var watchFontSize = ektj$('#tabs-watch-font-size').get(0) || ektj$('<span id="tabs-watch-font-size">M</span>').css({display: 'block', position: 'absolute', visibility: 'hidden'}).appendTo(document.body).get(0);
        ...
        tabs.bind('triggerTab', function() {
            ...
            var li = ektj$(this).parents('li:eq(0)');
            ...
            if (ektj$.browser.msie) {
                ...
                ektj$(this).trigger('click');
                if (settings.bookmarkable) {
                    ektj$.ajaxHistory.update(hash);
                    location.hash = hash.replace('#', '');
                }

            } else if (ektj$.browser.safari) {

                // Simply setting location.hash puts Safari into the eternal load state... ugh! Submit a form instead.
                var tempForm = ektj$('<form action="' + hash + '"><div><input type="submit" value="h" /></div></form>').get(0); // no need to append it to the body
                tempForm.submit(); // does not trigger the form's submit event...
                ektj$(this).trigger('click'); // ...thus do stuff here
                if (settings.bookmarkable) {
                    ektj$.ajaxHistory.update(hash);
                }
        ...
        tabs.bind('disableTab', function() {
            var li = ektj$(this).parents('li:eq(0)');
            if (ektj$.browser.safari) { /* fix opacity of tab after disabling in Safari... */
        ...
        tabs.bind('enableTab', function() {
            ...
            if (ektj$.browser.safari) { /* fix disappearing tab after enabling in Safari... */
        ...
        tabs.bind('click', function(e) {
            ...
            var clicked = this, li = ektj$(this).parents('li:eq(0)'), toShow = ektj$(this.hash), toHide = containers.filter(':visible');
            ...
            if (toShow.size()) {
                ...
                if (ektj$.browser.msie && settings.bookmarkable) {
                ...
                if (!ektj$.browser.msie) { // not in IE to prevent ClearType font issue
                    resetCSS['opacity'] = '';
                }
                ...
                function switchTab() {
                    if (settings.bookmarkable && trueClick) { // add to history only if true click occured, not a triggered click
                        ektj$.ajaxHistory.update(clicked.hash);
                    }
                    toHide.animate(hideAnim, hideSpeed, function() { //
                        ektj$(clicked).parents('li:eq(0)').addClass(settings.selectedClass).siblings().removeClass(settings.selectedClass);
                ...
                if (!settings.remote) {
                    ...
                    ektj$(clicked).trigger('loadRemoteTab', [switchTab]);
                }
        ...
        if (settings.bookmarkable) {
            ektj$.ajaxHistory.initialize(function() {
 ...
 * itself were clicked.
 *
 * @example ektj$('#container').triggerTab(2);
 * @desc Activate the second tab of the tab interface contained in <div id="container">.
 * @example ektj$('#container').triggerTab(1);
 * @desc Activate the first tab of the tab interface contained in <div id="container">.
 * @example ektj$('#container').triggerTab();
 * @desc Activate the first tab of the tab interface contained in <div id="container">.
 * @example ektj$('#container').triggerTab('fragment-2');
 ...
 *
 * @example ektj$('#container').disableTab(2);
 ...
 *
 * @example ektj$('#container').enableTab(2);
...
for (var i = 0; i < tabEvents.length; i++) {
    ektj$.fn[tabEvents[i]] = (function(tabEvent) {
        return function(tab) {
            return this.each(function() {
                var nav = ektj$('ul.tabs-nav' , this);
                nav = nav.size() && nav || ektj$('>ul:eq(0)', this); // fallback to default structure
                var a;
                if (!tab || typeof tab == 'number') {
                    a = ektj$('li a', nav).eq((tab && tab > 0 && tab - 1 || 0)); // fall back to 0
                } else if (typeof tab == 'string') {
                    a = ektj$('li a[@href$="#' + tab + '"]', nav);
                }
                a.trigger(tabEvent);
            });
        };
 ...
 *
 * @example ektj$('#container').activeTab();
