...
function makeMenuNS(isChild,menuCount,parMenu,parItem) {
	...
	if (!isChild) {
		...
		menu = makeElement("eMPMenu" + menuCount,tempWidth,null,null);
	}
	else {
		menu = makeElement("eMPMenu" + menuCount,null,parMenu,null);
	}
...
function makeMenuIE(isChild,menuCount,parMenu) {
	menu = makeElement("eMPMenu" + menuCount);
...
function itemSetup(whichItem,whichArray) {
	...
	if (IE4 && this.hasMore) {
		this.child = eval("eMPMenu" + this.id.substr(4));
...
function hideAll() {
	for(i=1; i<topCount; i++) {
		temp = eval("eMPMenu" + i + ".startChild");
		temp.isOn = false;
