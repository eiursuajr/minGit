...
PmPt.doVis = function(mN, show) {
  ...
  try {
   if( !show ) {
	  ...
	  var delay = menu[mN][0].isRoot && !thisI.sm ? 50 : hideDelay;
		setTimeout(function(){
                    if ("undefined" != typeof $ektron) { $ektron(document).trigger("ektron.dhtmlMenu.hide", [this.myName]); }
		}, delay);
	  	
   }
   else {
                if ("undefined" != typeof $ektron) { $ektron(document).trigger("ektron.dhtmlMenu.show", [this.myName]); }
