...
// Set some variables (know what we are dealing with)
	...
	if(opts) {
		if(opts.classNames) {
			opts.classNames = $.extend(defaults.classNames, opts.classNames); }
	...
	Finder.prototype.select = function(target,noCache,actionType) {
		...
		if(typeof target.length != 'number') {
			throw 'Ektron Finder: Target must be either a URL or a jQuery/DOM element'; return this; }

		if(!targetElement[0]) {
			throw 'Ektron Finder: Target element does not exist'; return this; }
	...
	//	else if (!finderId && method) {	throw 'Ektron Finder: Element is not a finder';	}
	});
};})($ektron);
