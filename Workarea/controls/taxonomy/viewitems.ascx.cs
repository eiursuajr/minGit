...
using Microsoft.Security.Application;
	public partial class viewitems : System.Web.UI.UserControl
	{
		...
		private void PopulateCommunityGroupGridData(DirectoryAdvancedGroupData cgDirectory)
		{
			...
			if ((cgDirectory != null)&& (cgDirectory.DirectoryItems != null)&& cgDirectory.DirectoryItems.Length > 0)
			{
				...
				foreach (CommunityGroupData item in cgDirectory.DirectoryItems)
				{
					...
					dr["COMMUNITYGROUP"] = "<img src=\"" + (item.GroupImage != "" ? item.GroupImage : this.m_refCommon.AppImgPath + "member_default.gif") + "\" align=\"left\" width=\"55\" height=\"55\" />";
		...
		private void TaxonomyToolBar()
		{
			...
			{
				result.Append(m_refstyle.GetButtonEventsWCaption(AppImgPath + "../UI/Icons/back.png", (string)("taxonomy.aspx?action=viewcontent&view=item&taxonomyid=" + TaxonomyId), m_refMsg.GetMessage("alt back button text"), m_refMsg.GetMessage("btn back"), "", StyleHelper.BackButtonCssClass, true));
			...
			if (m_strPageAction != "removeitems")
			{
				result.Append(m_refstyle.GetButtonEventsWCaption(AppImgPath + "../UI/Icons/add.png", (string) ("taxonomy.aspx?action=additem&taxonomyid=" + TaxonomyId + "&parentid=" + TaxonomyParentId), m_refMsg.GetMessage("lbl assign items"), m_refMsg.GetMessage("lbl assign items"), "", StyleHelper.AddButtonCssClass, true));
				result.Append(m_refstyle.GetButtonEventsWCaption(AppImgPath + "../UI/Icons/remove.png", (string) ("taxonomy.aspx?action=removeitems&taxonomyid=" + TaxonomyId + "&parentid=" + TaxonomyParentId), m_refMsg.GetMessage("remove taxonomy items"), m_refMsg.GetMessage("remove taxonomy items"), "", StyleHelper.RemoveButtonCssClass));
				result.Append(m_refstyle.GetButtonEventsWCaption(AppImgPath + "../UI/Icons/properties.png", (string) ("taxonomy.aspx?action=viewattributes&taxonomyid=" + TaxonomyId + "&parentid=" + TaxonomyParentId + "&LangType=" + TaxonomyLanguage), m_refMsg.GetMessage("btn properties"), m_refMsg.GetMessage("btn properties"), "", StyleHelper.ViewPropertiesButtonCssClass));
			...
			{
                result.Append(m_refMsg.GetMessage("view in label") + ":&nbsp;" + GetLanguageForTaxonomy(TaxonomyId, "", true, false, "javascript:LoadLanguage(this.value);") + "&nbsp;" + addDD + "<br>");
