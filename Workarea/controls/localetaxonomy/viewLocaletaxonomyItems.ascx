<%@ Control Language="C#" AutoEventWireup="true" CodeFile="viewLocaletaxonomyItems.ascx.cs" 
Inherits="viewLocaletaxonomyItems" %>

<script type="text/javascript">
    function LoadLanguage(inVal) {
		if(inVal=='0') { return false ; }
		document.location = 'LocaleTaxonomy.aspx?action=view&view=<%=m_strViewItem%>&taxonomyid='+<%=TaxonomyId%>+'&LangType=' + inVal ;
	}
    ...
    function LoadViewType(type){
	    document.location='LocaleTaxonomy.aspx?iframe=true&action=view&view='+type+'&folderid=0&taxonomyid=<%=TaxonomyId %>&parentid=<%=TaxonomyParentId %>' ;
    }
