...
function createNode(node)
{
    ...
    span_elem.innerHTML = "<img onclick=\"deleteNode('" + makeSafeId(node.title) + "');\" alt=\"" + BreadcrumbRemoveTooltip + "\" src=\"images/UI/Icons/delete.png\" />&nbsp;&nbsp;&nbsp;&nbsp;";
...
{    
    ...
    document.getElementById("btnAddSitepath").value = BreadcrumbSaveBtnText;
}
function deleteNode(id)
{
    ...
    if($ektron('div#sitemap_nodes > div').length < 2) { $ektron('#dvUpDownArrow')[0].style.display='none';}
...
function clearSitemapForm()
{
    ...
    document.getElementById("btnAddSitepath").value = BreadcrumbAddBtnText;
...
function InheritSitemapPath(bChecked)
{
    ...
    if (bChecked){        
        ...
        if (confirm(addFolderBreadCrumb_breakInheri)) {
