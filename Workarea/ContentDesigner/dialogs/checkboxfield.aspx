...
<%@ Page language="c#" CodeFile="checkboxfield.aspx.cs" Inherits="Ektron.ContentDesigner.Dialogs.CheckboxField" AutoEventWireup="false" %>
		            ...
		            <tr>
		                <td colspan="3">
		                    <fieldset class="Ektron_TopSpaceVeryVerySmall Ektron_BottomSpaceSmall">
		                        <legend title="Default value" id="lblDefVal" runat="server">Default value</legend>
		                        <div>
		                            <input type="radio" title="Set the Default Value to <%=GetMessage("lbl def val true") %>" name="optDefVal" id="optDefValT" value="True" /><label for="optDefValT" id="lblDefValT" runat="server" title="True (checked)">True (checked)</label><br />
		                            <input type="radio" title="Set the Default Value to <%=GetMessage("lbl def val false") %>" name="optDefVal" id="optDefValF" value="False" checked="checked" /><label for="optDefValF" id="lblDefValF" runat="server" title="False (unchecked)">False (unchecked)</label><br />
		                        </div>
		                    </fieldset>
		                </td>
		            </tr>
		            <tr>
		                <td><label for="txtCaption" class="Ektron_StandardLabel" id="lblCaption" runat="server" title="Caption">Caption:</label></td>
		                <td colspan="2"><input type="text" title="<%=GetMessage("lbl caption c") %>" name="txtCaption" id="txtCaption" value="" class="Ektron_StandardTextBox" /></td>
		            </tr>
	            </table>
	        </radTS:PageView>
...
<!--
	...
	function initField()
	{
		...
		ekFieldAdvancedControl.setDefaultVal("true");
	...
	function insertField()
	{
		...
		if (null == oFieldElem)
		{
			if (window.radWindow && window.radWindow.IsIE)
			{
                ...
                oFieldElem = $ektron("<input type=\"checkbox\" name=\"" + ekFieldNameControl.getName() + "\" />").get(0);
			}
			else
			{
