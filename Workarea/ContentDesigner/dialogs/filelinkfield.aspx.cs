...
{
    /// <summary>
    /// Summary description for FileLinkField.
    /// </summary>
    public partial class FileLinkField : WorkareaDialogPage
    {
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
			...
			this.Title.Text = this.GetMessage("lbl link field");
			this.RadTabStrip1.Tabs[0].Text = this.GetMessage("tab general");
            this.RadTabStrip1.Tabs[0].ToolTip = this.GetMessage("tab general");
			this.RadTabStrip1.Tabs[1].Text = this.GetMessage("tab data style");
            this.RadTabStrip1.Tabs[1].ToolTip = this.GetMessage("tab data style"); 
			this.RadTabStrip1.Tabs[2].Text = this.GetMessage("tab advanced");
            this.RadTabStrip1.Tabs[2].ToolTip = this.GetMessage("tab advanced"); 
			this.lblDefault.InnerHtml = this.GetMessage("lbl default");
			this.lblLocation.InnerHtml = this.GetMessage("lbl location");
			this.lblCannotBeBlank.InnerHtml = this.GetMessage("lbl cannot be blank");
			this.cmdSelect.Text = this.GetMessage("lbl select ellipsis");
            this.cmdSelect.ToolTip = this.GetMessage("lbl select ellipsis");
			this.lblDescription.InnerHtml = this.GetMessage("description label");
			this.lblTargetFrame.InnerHtml = this.GetMessage("lbl target frame c");
			this.lblNewWindow.InnerHtml = this.GetMessage("lbl new window");
			this.lblSameWindow.InnerHtml = this.GetMessage("lbl same window");
			this.lblParentWindow.InnerHtml = this.GetMessage("lbl parent window");
			this.lblBrowserWindow.InnerHtml = this.GetMessage("lbl browser window");
			this.sSelectFile.Text = this.GetMessage("lbl select file");
